...
D3D11PFX_HDR::D3D11PFX_HDR(D3D11PfxRenderer* rnd) : D3D11PFX_Effect(rnd)
{
	...
	engine->GetContext()->ClearRenderTargetView(LumBuffer1->GetRenderTargetView(), float4(0, 0, 0, 0).toPtr());
	engine->GetContext()->ClearRenderTargetView(LumBuffer2->GetRenderTargetView(), float4(0, 0, 0, 0).toPtr());
	engine->GetContext()->ClearRenderTargetView(LumBuffer3->GetRenderTargetView(), float4(0, 0, 0, 0).toPtr());
	ActiveLumBuffer = 0;
}
