...
void D3D11PFX_SMAA::RenderPostFX(ID3D11ShaderResourceView * renderTargetSRV)
{
	...
	engine->GetContext()->ClearRenderTargetView(EdgesTex->GetRenderTargetView(), float4(0, 0, 0, 0).toPtr());
	engine->GetContext()->ClearRenderTargetView(BlendTex->GetRenderTargetView(), float4(0, 0, 0, 0).toPtr());
