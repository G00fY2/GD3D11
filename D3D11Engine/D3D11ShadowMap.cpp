...
void D3D11ShadowMap::Resize( int size ) {
    ...
    if ( !m_device ) return;
    const auto numCascades = GetNumCascades();

    int s = std::min<int>( std::max<int>( size, 512 ), (FeatureLevel10Compatibility ? 8192 : 16384) );
    m_worldShadowmaps.clear();
    m_worldShadowmaps.reserve( numCascades );
...
XRESULT D3D11ShadowMap::DrawLighting( std::vector<VobLightInfo*>& lights ) {
    ...
    const auto splits = ComputeCascadeSplits( nearPlane, farPlane, GetNumCascades(), 0.90f );
    ...
    if ( !isOutdoor ) {
        if ( Engine::GAPI->GetRendererState().RendererSettings.EnableShadows && lastBspMode == zBSP_MODE_OUTDOOR ) {
            for ( size_t cascadeIdx = 0; cascadeIdx < GetNumCascades(); ++cascadeIdx ) {
        ...
        for ( size_t i = 0; i < GetNumCascades(); ++i ) {
        ...
        for ( size_t cascadeIdx = 0; cascadeIdx < GetNumCascades(); ++cascadeIdx ) {
            ...
            float splitRatio = splits[cascadeIdx + 1] / splits[GetNumCascades()];
