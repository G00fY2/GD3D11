...
bool GVegetationBox::PositionInsideBox(const Vector3 & p)
{
	if (p.x > BoxMin.x &&
		p.y > BoxMin.y &&
		p.z > BoxMin.z &&
								...
								zCTexture * meshTexture)
{
	...
	BoxMax = Vector3(-FLT_MAX, -FLT_MAX, -FLT_MAX);
	BoxMin = Vector3(FLT_MAX, FLT_MAX, FLT_MAX);
...
XRESULT GVegetationBox::InitVegetationBox(const Vector3 & min, 
										  const Vector3 & max, 
										  const std::string & vegetationMesh, 
										  float density, 
										  float maxSize,
										  ...
										  EShape shape)
{
	...
	std::vector<Vector3> polysInside;
	...
	for(int i=0;i<Engine::GAPI->GetLoadedWorldInfo()->BspTree->GetNumPolys();i++)
	{
		for(int v=0;v<4;v++)
		{
			...
			{
				// Check center too
				Vector3 tri[] = {   p[i]->getVertices()[0]->Position,
...
void GVegetationBox::InitSpotsRandom(const std::vector<Vector3> & trisInside, EShape shape, float density)
{
	Vector3 mid = BoxMin * 0.5f + BoxMax * 0.5f;
	Vector3 bs = (BoxMax - BoxMin);
	...
	for(unsigned int i=0;i<trisInside.size();i+=3)
	{
		for(unsigned int d=0;d<std::max(1.0f, 30 * density);d++)
		{
			...
			if (PositionInsideBox(rnd))
			{
				...
				{
					Vector2 m2 = Vector2(mid.x, mid.z);
					float dist = (Vector2(rnd.x, rnd.z) - m2).Length();
	...
	{
		Matrix w = XMMatrixTranslation(spots[i].x, spots[i].y, spots[i].z);
		float scale = Toolbox::lerp(20, 80, Toolbox::frand());
		XMMATRIX s = XMMatrixScaling(scale, scale, scale);
		XMMATRIX r = XMMatrixRotationY(Toolbox::frand() * DirectX::XM_PI * 2.0f);

		w = XMMatrixMultiplyTranspose(XMMatrixMultiply(r, s), w);
	...
	InstancingBuffer->Init(&VegetationSpots[0], VegetationSpots.size() * sizeof(Matrix));
...
void GVegetationBox::RenderVegetation(const Vector3 & eye)
{
	...
	Vector3::Up;
	*gcb.G_NormalVS.toXMFLOAT3() = Vector3::TransformNormal(Vector3::Up, view);
	...
	// Draw the batch
	VegetationMesh->DrawBatch(InstancingBuffer, VegetationSpots.size(), sizeof(Matrix));
...
void GVegetationBox::VisualizeGrass(const float4 & color)
{
	// Draw bounding box
	...
	for(unsigned int i=0;i<VegetationSpots.size();i++)
	{
		...
		Vector3 spot = Vector3(VegetationSpots[i]._14, VegetationSpots[i]._24, VegetationSpots[i]._34); 
		Vector3 scale = Vector3(0, 0, 0);
		Vector4* m = (Vector4 *)&VegetationSpots[i];

		// Compute scale
		//scale.x = D3DXVec3Length((Vector3 *)&m[0]);
		scale.y = ((Vector3*)&m[1])->Length();
		//scale.z = D3DXVec3Length((Vector3 *)&m[2]);
...
void GVegetationBox::GetBoundingBox(Vector3 * bbMin, Vector3 * bbMax)
{
	*bbMin = BoxMin;
	*bbMax = BoxMax;
}

void GVegetationBox::SetBoundingBox(const Vector3 & bbMin, const Vector3 & bbMax)
{
	BoxMin = bbMin;
	BoxMax = bbMax;
}
...
void GVegetationBox::RemoveVegetationAt(const Vector3 & position, float range) {
	// Make a list of the vector
	std::list<Matrix> s(VegetationSpots.begin(), VegetationSpots.end());

	// Remove everything in range
	for (auto it = s.begin(); it != s.end();) {
		Vector3 spot = Vector3(it->_14, it->_24, it->_34); 

		float d = (spot - position).Length();
...
void GVegetationBox::ApplyUniformScaling(float scale)
{
	...
	{
		Matrix w;
		VegetationSpots[i].Transpose(w);
		VegetationSpots[i] = s * w;
		VegetationSpots[i].Transpose(VegetationSpots[i]);
	}
...
void GVegetationBox::SaveToFILE(FILE* f, int version)
{
	...
	std::vector<Vector4> spots;
	for(unsigned int i=0;i<VegetationSpots.size();i++)
	{
		Vector4* m = (Vector4 *)&VegetationSpots[i];
		Vector4 spot = Vector4(VegetationSpots[i]._14, VegetationSpots[i]._24, VegetationSpots[i]._34, ((Vector3 *)&m[1])->Length());

		spots.push_back(spot);
	}

	fwrite(&spots[0], sizeof(Vector4) * vsize, 1, f);

	// Save trisInside
	int tsize = TrisInside.size();
	fwrite(&tsize, sizeof(tsize), 1, f);
	fwrite(&TrisInside[0], sizeof(Vector3) * tsize, 1, f);
...
void GVegetationBox::LoadFromFILE(FILE* f, int version)
{
	...
	std::vector<Vector4> spots;
	spots.resize(vsize);
	fread(&spots[0], sizeof(Vector4) * vsize, 1, f);
