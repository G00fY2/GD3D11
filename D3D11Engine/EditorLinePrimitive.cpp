...
HRESULT EditorLinePrimitive::CreateSolidBoxPrimitive(float4& Color, float Extends)
{
	...
	XMVECTOR extends = XMVectorSet(Extends, Extends, Extends, 1);
	// Loop through all vertices and apply the extends
	for(i = 0; i < 36; i++)
	{
		XMVECTOR pos = XMVectorMultiply(
			XMLoadFloat3(vx[i].Position.toXMFLOAT3()),
			extends);
		XMStoreFloat3(vx[i].Position.toXMFLOAT3(), pos);
	}
...
void XM_CALLCONV EditorLinePrimitive::SetWorldMatrix(FXMMATRIX World, FXMVECTOR Loc, FXMVECTOR Rot, CXMVECTOR Scale)
{
