...
float Gauss()
{
	...
	if (u1 < 1e-6f)
		u1 = 1e-6f;
	return sqrtf(-2 * logf(u1)) * cosf(2*DirectX::XM_PI * u2);
}
...
void OceanSimulator::initHeightMap(OceanParameter& params, DirectX::SimpleMath::Vector2* out_h0, float * out_omega)
{
	...
	DirectX::SimpleMath::Vector2 wind_dir = DirectX::XMVector2Normalize(params.wind_dir);
	...
	for (i = 0; i <= height_map_dim; i++)
	{
		...
		K.y = (-height_map_dim / 2.0f + i) * (2 * (DirectX::XM_PI) / patch_length);

		for (j = 0; j <= height_map_dim; j++)
		{
			K.x = (-height_map_dim / 2.0f + j) * (2 * (DirectX::XM_PI) / patch_length);
